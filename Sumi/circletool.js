...
function updateTooltip(x, y, circle) {
    $('#circle-coords').textContent = `Center: (${circle.x.toFixed(2)}, ${circle.y.toFixed(2)}), Radius: ${circle.radius.toFixed(2)}`;
}
...
canvas.addEventListener('mousedown', (e) => {
    if (isDrawingCircle) {
        ...
        if (!currentCircle) {
            ...
            // Reset drawing state
            window.appendToEditor(`_.stroke(_.circle(${startX.toFixed(2)}, ${startY.toFixed(2)}, ${currentCircle.radius.toFixed(2)}));`)
